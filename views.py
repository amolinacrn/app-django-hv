        ...
        if info["queryset"].count() != 0:
            for modelo in info["queryset"]:
                ...
                if archivo:
                    ...
                    if pdf_path:
                        pages = convert_from_path(pdf_path, dpi=200, poppler_path="/usr/bin")
                    imagenes_base64 = []
                    for page in pages:
                        buffer = BytesIO()
                    ...
                    matriz_imagenes_base64.append((imagenes_base64,modelo.link,request.build_absolute_uri(archivo.url)))
